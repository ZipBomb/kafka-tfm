...
# Performs hyperparameter tuning over RandomForestRegressor scikit-learn implementation, logs metrics and stores results
def rf_tuning(n_estimators = [10, 11, 1], k = 5,
    ...
    if save_model:
        ...
        final_model = Pipeline(steps = [('scaler', StandardScaler()),
                                        ('regression', RandomForestRegressor(
                                            n_estimators = param_list[best_index]['regression__n_estimators'],
                                            n_jobs = -1))])
...
def check_valid_estimators(value):
    try:
        ...
        if n_estimators < 1:
            ...
            return [n_estimators, n_estimators + 1, 1]
    except ValueError:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog = 'random_forest.py',
        description = "Performs hyperparameter CV tuning for scikit-learn's Random Forest implementation to solve "
                    + "Kaggle's 'Quality prediction in a mining process' problem and logs the experiment with mlflow.",
        formatter_class = argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument('-ne', '--n_estimators', type = check_valid_estimators, metavar = 'N', default = [10, 11, 1],
                        help = 'range for the number of trees in the ensemble')
    ...
    rf_tuning(n_estimators = args.n_estimators,
              k = int(args.k_folds),
